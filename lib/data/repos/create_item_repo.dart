...
class CreateItemRepo{
  ...
  Future<Either<Failure, CountryModel>> getCountry() async {
    try {
      ...
      var data = await ApiService.get(
          endPoint: 'adminGetAllNations',);

      model = CountryModel.fromJson(data);
      return Right(model);
    } catch (e) {
  ...
  Future<Either<Failure, CitiesModel>> getCity({required int countryId}) async {
    try {
      CitiesModel? model;
      var data = await ApiService.get(
  ...
  Future<Either<Failure, String>> createHotel({required Map<String,dynamic> data}) async {
    try {

      var response = await ApiService.post(
        endPoint: 'adminCreateHotel',data: data);
